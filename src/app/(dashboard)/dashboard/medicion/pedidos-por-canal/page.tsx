  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      (doc as any).autoTable({
        ...
        body: channelShares.map(s => [s.name, s.count, s.totalAmount.toLocaleString(), `${s.percentage.toFixed(1)}%`]),
      });
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={handleExportExcel} disabled={isExporting !== null} className="gap-2">
            <FileSpreadsheet className="h-4 w-4" /> Excel
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportPDF} disabled={isExporting !== null} className="gap-2">
                  ...
                  <TableBody>
                    {channelShares.map((share, i) => (
                      <TableRow key={share.name}>
                        ...
                        <td className="text-center font-bold text-xs">{share.count}</td>
                        <td className="text-right pr-4 text-xs font-black text-primary">{formatCurrency(share.totalAmount)}</td>
                      </tr>
                    ))}
                  </TableBody>
                </Table>
                 ...
                 {isLogsLoading ? (
                   ...
                   filteredLogs.map(event => (
                     <TableRow key={event.trackingId}>
                        ...
                        <td className="text-right font-bold text-xs">{formatCurrency(event.total)}</td>
                        <td className="text-center text-[10px]">
                           {event.createdAt?.toDate ? format(event.createdAt.toDate(), "dd/MM/yyyy HH:mm") : '---'}
                        </td>
                        <td className="pr-6">
                          <Button variant="ghost" size="sm" className="h-8" onClick={() => handleOpenDetail(event.invoiceId)}>
                            <Eye size={14} />
                          </Button>
                        </td>
                     </TableRow>
