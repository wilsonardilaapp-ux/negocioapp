...
const defaultShareConfig: Omit<MenuShare, 'id' | 'businessId' | 'createdAt' | 'updatedAt'> = {
  ...
  slugLanding: '',
  useCustomSlug: false,
  useCustomSlugLanding: false,
  socialPreviewImageUrl: null,
  socialShareMessage: '¡Hola! Te invito a conocer nuestro sitio oficial ✨',
  qrConfig: {
    size: 256,
...
export default function ShareLandingPage() {
  ...
  const landingUrl = useMemo(() => {
    ...
    const slugToUse = shareConfig.useCustomSlugLanding ? (shareConfig.slugLanding || user.uid) : user.uid;
  ...
  const downloadQR = () => {
    ...
    html2canvas(qrCodeRef.current, { backgroundColor: null }).then((canvas) => {
        ...
        link.download = 'qr-landing-page.png';
      ...
      <Card>
        <CardHeader className="flex flex-row justify-between items-center">
          <div className="space-y-1">
            <CardTitle>Compartir Landing Page</CardTitle>
            <CardDescription>Gestiona el acceso público a tu página de aterrizaje oficial.</CardDescription>
          </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <Button onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent((shareConfig.socialShareMessage || '') + ' ' + landingUrl)}`, '_blank')}><WhatsAppIcon className="mr-2" /> WhatsApp</Button>
            </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap items-center gap-2">
            ...
            <Button
                ...
                onClick={() => window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent((shareConfig.socialShareMessage || '') + ' ' + landingUrl)}`, '_blank')}
            >
