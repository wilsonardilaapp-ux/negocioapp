...
export default function BackupsPage() {
    ...
    // This is a superadmin page, so it should list backups for ALL businesses.
    // However, for simplicity and to avoid complex queries across collections
    // which are not well-supported by Firestore security rules for `list` operations,
    // we'll demonstrate with a single, hardcoded businessId for now.
    // In a real app, this would require a backend function or a different data model.
    const DEMO_BUSINESS_ID = user?.uid; // Using current user's ID as a stand-in for a selected business

    const backupsQuery = useMemoFirebase(() => {
        if (!firestore || !DEMO_BUSINESS_ID) return null;
        return query(collection(firestore, `businesses/${DEMO_BUSINESS_ID}/backups`), orderBy('createdAt', 'desc'), limit(10));
    }, [firestore, DEMO_BUSINESS_ID]);
            ...
            <Card>
                <CardHeader className="flex flex-row justify-between items-center">
                    <div>
                        ...
                        <p className="text-muted-foreground">Administra y monitorea los respaldos de la plataforma.</p>
                    </div>
