...
interface ChatbotMenuConfigContentProps {
  businessId: string;
}

export function ChatbotMenuConfigContent({ businessId: activeBusinessId }: ChatbotMenuConfigContentProps) {
  const { user } = useUser();
                        ...
                        <>
                          ...
                          {isGlobalActive && (
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                                <button className="text-white hover:scale-110 transition-transform" onClick={() => document.getElementById('avatar-upload')?.click()}><Pencil className="h-6 w-6" /></button>
                            </div>
                          )}
                        </>
