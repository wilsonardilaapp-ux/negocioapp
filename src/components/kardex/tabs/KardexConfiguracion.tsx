            ...
            <Card>
                ...
                <CardContent className="space-y-6">
                    <div><Label>Método de Valuación de Inventario</Label><RadioGroup value={config.metodoValuacion} onValueChange={(value: MetodoValuacion) => handleConfigChange('metodoValuacion', value)} className="mt-2">
                        ...
                        {false && <div className="flex items-center space-x-2"><RadioGroupItem value="peps" id="peps" /><Label htmlFor="peps">PEPS (Primeras en Entrar, Primeras en Salir)</Label></div>}
                        {false && <div className="flex items-center space-x-2"><RadioGroupItem value="ueps" id="ueps" /><Label htmlFor="ueps">UEPS (Últimas en Entrar, Primeras en Salir)</Label></div>}
                    </RadioGroup></div>
                    {false && <div className="flex items-center justify-between rounded-lg border p-4"><Label htmlFor="asiento-auto" className="flex flex-col space-y-1"><span>Generar Asiento Contable Automático</span><span className="font-normal leading-snug text-muted-foreground">Al guardar una venta, se creará el asiento de costo de ventas.</span></Label><Switch id="asiento-auto" checked={config.generarAsientoAutomatico} onCheckedChange={(val) => handleConfigChange('generarAsientoAutomatico', val)} /></div>}
